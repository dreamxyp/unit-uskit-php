...
namespace Baidu\Iov\Kit\Session;
        ...
        switch ($conf['type']) {
            ...
            case 'custom':
                ...
                if(!class_exists($class)) {
                    throw new UsException("Session class '$class' doesn't exist.");
                }
                $session = new $class($logger, $conf);
                if(!$session instanceof AbstractSession) {
                    throw new UsException('Session class should extend Baidu\Iov\Kit\Session\AbstractSession');
                }
                break;
            default:
                $session = new FileSession($logger, $conf);
